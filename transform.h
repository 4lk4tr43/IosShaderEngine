...
public:
	...
	bool IsIdentity(float margin_position = 0.0f, float margin_rotation = 0.0f)
	{
		return (position.x + position.y + position.z) <= margin_position && (rotation.x + rotation.y + rotation.z - 1.0f) <= margin_rotation;
	}
